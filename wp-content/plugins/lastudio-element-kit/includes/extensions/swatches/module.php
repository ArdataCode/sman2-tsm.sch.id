...
class Module extends Module_Base {
    ...
    public function register_ajax_actions( $ajax_manager ){
        $ajax_manager->register_ajax_action( 'swatches_get_variation_attributes', [ $this, 'ajax_swatches_get_variation_attributes' ], true );
        $ajax_manager->register_ajax_action( 'swatches_get_product_variations', [ $this, 'ajax_get_product_variations' ], false );
    }
            ...
            <div class="lakit_variation_thumb lakit_variation_thumb--<?php echo $variation_id; ?>">
                ...
                <ul class="lakit_variation_thumbs"><?php if (!empty($attachmentsExp)) {
                    foreach ($attachmentsExp as $id) {
                        $image_ids[] = $id; ?><li class="image" data-attachment_id="<?php echo $id; ?>">
...
AND rMeta.meta_value > 0;
    ...
    public function render_360_and_video_metabox( $form_fields, $post ){
        ...
        ob_start();
        ?>
        <div class="lakit-admin__attach-media-wrap" data-test="<?php echo esc_attr(json_encode([
	        'id' => $post->ID,
	        'type' => $type
        ])); ?>">
            ...
            <div class="frm-fields">
                ...
                <div class="frm-field frm-field--depends" data-condition="video" style="<?php echo $type !== 'video' ? 'display: none' : '' ?>">
                    <label for="attachments-<?php echo $post->ID ?>-lakit_attach__videourl">Media URL</label>
                    <input type="text" id="attachments_<?php echo $post->ID ?>_lakit_attach__videourl" name="attachments[<?php echo $post->ID ?>][lakit_attach__videourl]" value="<?php echo esc_attr($video_url); ?>"/>
                    <button type="button" class="button" data-id="attachments_<?php echo $post->ID ?>_lakit_attach__videourl" data-type="video">Attach MP4</button>
                </div>
                <div class="frm-field frm-field--depends" data-condition="threesixty" style="<?php echo $type !== 'threesixty' ? 'display: none' : '' ?>">
                    ...
                    <input type="hidden" id="attachments_<?php echo $post->ID ?>_lakit_attach__sprite" name="attachments[<?php echo $post->ID ?>][lakit_attach__sprite]" value="<?php echo esc_attr($sprite); ?>"/>
                    <button type="button" data-id="attachments_<?php echo $post->ID ?>_lakit_attach__sprite" class="button" data-type="image">Select Image</button>
                </div>
                <div class="frm-field frm-field--depends" data-condition="threesixty" style="<?php echo $type !== 'threesixty' ? 'display: none' : '' ?>">
