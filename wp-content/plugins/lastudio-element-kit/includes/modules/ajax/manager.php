...
if (!class_exists('LaStudio_Kit_Ajax_Manager')) {
    ...
    class LaStudio_Kit_Ajax_Manager
    {
    	...
    	public function __construct() {
			...
			add_filter( 'nonce_life', [ $this, 'nonce_life' ], 20, 2 );
		}
	     ...
	     */
		public function nonce_life( $value, $action ){
			if( self::NONCE_KEY === $action ){
	     ...
	     * @param boolean $protected protected.
	     */
	    public function register_ajax_action( $tag = '', $callback = null, $protected = true ) {
		    ...
		    $this->ajax_actions[ $tag ] = compact( 'tag', 'callback', 'protected' );
	    }
...
//			    $this->add_response_data( false, esc_html__( 'Token Expired.', 'lastudio-kit' ) )->send_error( 401 );
//		    }

		    if(empty($_REQUEST['actions'])){
			    $this->current_action_id = 'error';
			    $this->add_response_data( false, esc_html__( 'Action not found.', 'lastudio-kit' ) )->send_error( 401 );
		    }
		    ...
		    foreach ( $this->requests as $id => $action_data ) {
			    ...
			    if ( ! isset( $this->ajax_actions[ $action_data['action'] ] ) ) {
				    $this->add_response_data( false, esc_html__( 'Action not found.', 'lastudio-kit' ), 401 );
				    continue;
			    }
			    $current_ajax_action = $this->ajax_actions[ $action_data['action'] ];
				if(!empty($current_ajax_action['protected']) && !$this->verify_request_nonce()){
					$this->add_response_data( false, esc_html__( 'Token Expired.', 'lastudio-kit' ), 401 );
					continue;
				}
			    try {
